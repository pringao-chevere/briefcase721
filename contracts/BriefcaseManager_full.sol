...
contract BriefcaseManager {
    ...
    uint[] BRIEFCASES;  //Array of all cases [tokenId,tokenId,...]
    mapping(uint256 => address) OWNERS;  //Mapping of token owners

    //    METADATA VARS
    string private __name = "TokenBriefcase";
    string private __symbol = "CASE";
    bytes private __uriBase = bytes("https://TODO");
    ...
    function mint() external returns(address) {
        ...
        ID_TO_INDEX[tokenId] = BRIEFCASES.length;
        BRIEFCASES.push(tokenId);
    ...
    function totalSupply() external view returns (uint256){
        return BRIEFCASES.length;
    }
    function tokenByIndex(uint256 _index) external view returns(uint256){
        require(_index < BRIEFCASES.length,"index");
        return BRIEFCASES[_index];
